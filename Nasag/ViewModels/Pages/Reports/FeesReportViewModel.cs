...
public sealed record FeesStatusFilterOption(string DisplayName, FeesReportStatusFilter Value);

public sealed partial class FeesReportViewModel : ObservableObject
{
        ...
        ICurrentUserService users)
    {
        ...
        StatusOptions =
        [
            new FeesStatusFilterOption("كل الحالات", FeesReportStatusFilter.All),
            new FeesStatusFilterOption("مسدّد بالكامل", FeesReportStatusFilter.FullyPaid),
            new FeesStatusFilterOption("دفع جزئي", FeesReportStatusFilter.PartiallyPaid),
            new FeesStatusFilterOption("غير مدفوع", FeesReportStatusFilter.Unpaid),
            new FeesStatusFilterOption("عليه متأخرات", FeesReportStatusFilter.HasOverdue),
        ];
    ...
    public IReadOnlyList<FeesStatusFilterOption> StatusOptions { get; }
    ...
    [ObservableProperty] private FeesStatusFilterOption? _selectedStatus;
    ...
    private FeesReportQuery BuildQuery() =>
        new(SelectedGrade?.Id, SelectedSection?.Id, SelectedStatus?.Value ?? FeesReportStatusFilter.All);
    ...
    private async Task PreviewAsync()
    {
        ...
        {
            IsBusy = true;
            var result = await _repo.GetFeesReportAsync(BuildQuery()).ConfigureAwait(true);
            var doc = FeesReportDocument.Build(result);
            PrintService.PreviewAndPrint(doc, "كشف الرسوم");
            ReportRecent(result);
        }
        catch (Exception ex)
        {
    ...
    private async Task ExportPdfAsync()
    {
        ...
        {
            Filter = "PDF (*.pdf)|*.pdf",
            FileName = $"كشف-الرسوم-{DateTime.Now:yyyy-MM-dd}.pdf",
            AddExtension = true,
        ...
        try
        {
            IsBusy = true;
            var result = await _repo.GetFeesReportAsync(BuildQuery()).ConfigureAwait(true);
            await _pdf.SaveFeesAsync(dlg.FileName, result).ConfigureAwait(true);
    ...
    private async Task ExportExcelAsync()
    {
        ...
        var dlg = new SaveFileDialog
        {
            ...
            FileName = $"كشف-الرسوم-{DateTime.Now:yyyy-MM-dd}.xlsx",
            AddExtension = true,
            OverwritePrompt = true,
        };
        if (dlg.ShowDialog() != true) return;
        ...
        try
        {
            IsBusy = true;
            var result = await _repo.GetFeesReportAsync(BuildQuery()).ConfigureAwait(true);
            await _excel.ExportFeesReportAsync(dlg.FileName, result).ConfigureAwait(true);
    ...
    private void ReportRecent(FeesReportResult r)
    {
        ...
        var desc = $"{grade} • {section} • {r.StatusLabelAr} • {r.Totals.StudentCount} طالب";
        OnReportGenerated?.Invoke(new RecentReportEntry(
            Guid.NewGuid(),
            RecentReportKind.Fees,
            "كشف الرسوم",
            desc,
            _users.DisplayName,
            DateTime.Now));
